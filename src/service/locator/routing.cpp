...
continuum_t::continuum_t(std::unique_ptr<logging::log_t> log, const std::map<std::string, unsigned int>& group):
    m_log(std::move(log))
{
...
continuum_t::get() const {
    point_type point = m_distribution(m_rng);
